...
// 分页
function lingfeng_custom_pagenavi( $custom_query,$total_files,$range =10 ) {
...
//                    product_cat_create(get_pagenum_link($i));
                    echo "<a href='".get_pagenum_link($i) ."'";
                    if($i==$paged) echo " class='current'";echo "class='page_num'>".$i."</a>";
                }
...
//                    var_dump('function_exists');
//                    product_cat_create(get_pagenum_link($i));
                    echo "<a href='".get_pagenum_link($i) ."'";if($i==$paged) echo " class='current page_num'";echo "class='page_num'>".$i."</a>";
                }
            }
        }else{
...
//                product_cat_create(get_pagenum_link($i));
                echo "<a href='".get_pagenum_link($i) ."'";
                if($i==$paged)echo " class='current page_num'";echo "class='page_num'>".$i."</a>";
            }
        }
...
// 分页样式函数
function fenyestyle($q, $total_files){
	return lingfeng_custom_pagenavi($q, $total_files);
}
/*
...
if( function_exists('acf_add_options_page') ) {
	...
	acf_add_options_page(
		array(
			'page_title' 	=> '联系我们配置页',
			'menu_title'	=> 'contact_us_option',
			'menu_slug' 	=> 'contact_us_option',
			'redirect'		=> false
		)
	);
...
function footer_shortcode(){
	$friendly_info = get_field('friendly_link','widget_text-2');
	$footer_html = '<div class="footer_wrap">';
	$footer_html .= '<div class="footer_top clearfix">';
	$footer_html .= '<div class="footer_left_wrap">';
	$footer_html .= '<a href="http://web.wanshixingwuye.com/?cat=8">集团概况</a>';
	$footer_html .= '<a href="http://web.wanshixingwuye.com/?page_id=18">下属公司</a>';
	$footer_html .= '<a href="http://web.wanshixingwuye.com/?cat=9">项目中心</a>';
	$footer_html .= '<a href="http://web.wanshixingwuye.com/?page_id=26">企业荣誉</a>';
	$footer_html .= '<a href="http://web.wanshixingwuye.com/?cat=5">企业招标</a>';
	$footer_html .= '<a href="http://web.wanshixingwuye.com/?page_id=30">加入我们</a>';
	$footer_html .= '<a href="http://web.wanshixingwuye.com/?page_id=32">联系我们</a>';
	$footer_html .= '</div>';
	$footer_html .= '<div class="footer_right_wrap">';
	$footer_html .= '<span>友情链接：</span>';
	foreach($friendly_info as $friendly_info_item){
		$footer_html .= "<a target='_blank' href='".$friendly_info_item['friendly_link_url']."'>".$friendly_info_item['friendly_link_title']."</a>";
	}
	$footer_html .= '</div>';
	$footer_html .= '</div>';
	$footer_html .= '<div class="footer_bottom">'.avia_get_option("copyright").'</div>';
	$footer_html .= '</div>';
	return $footer_html;
}
add_shortcode('footer_shortcode','footer_shortcode');
add_image_size('index_tab',241,187,true);
add_image_size('project_center',232,152,true);
add_image_size('index_right_silder',260,140,true);
